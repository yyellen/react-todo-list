...
const TodoForm = ({inputRef, addTodo}) => {
  return (
    <div className="w-72 mt-4 flex justify-center">
      <input type="text" ref={inputRef} className="w-64 h-8 p-2 border-[2px] rounded" placeholder="new task"/>
      <Button text="+" onClick={addTodo} className="w-8 h-8 ml-1 border-[2px] rounded"/>
    </div>
  );
}
